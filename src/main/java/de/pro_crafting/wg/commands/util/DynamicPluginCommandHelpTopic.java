package de.pro_crafting.wg.commands.util;
/*
...
public class DynamicPluginCommandHelpTopic extends HelpTopic {
    ...
    public DynamicPluginCommandHelpTopic(DynamicPluginCommand cmd) {
        ...
        if (cmd.getRegisteredWith() instanceof CommandInspector) {
            ...
            if (cmd.getRegisteredWith() instanceof Dispatcher) {
                Dispatcher registeredWith = (Dispatcher) cmd.getRegisteredWith();

                final String lookupName = cmd.getName().replaceAll("/", "");
                fullTextTemp = registeredWith.get(lookupName) != null ? registeredWith.get(lookupName).getDescription().getHelp() : null;
                registeredWith.get(lookupName);

                // No full help text, assemble help text from info
                if (registeredWith.get(cmd.getName()) != null) {
                    final String shortText = registeredWith.get(cmd.getName()).getDescription().getShortDescription();
